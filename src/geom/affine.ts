 ...
 * coordinates in [0,1]: u = (x + W/2)/W, v = (y + H/2)/H.
 *
 * This is purely geometric — it says nothing about the image's own
 * orientation, since `Face.mat` is baked when the face is created, before any
 * image is chosen. Image-vs-sheet orientation mismatch (e.g. a landscape
 * photo on a portrait sheet) is corrected at draw time instead, where both
 * the sheet and the current image are known — see `coverCrop` in
 * `render/paper.ts`.
 */
