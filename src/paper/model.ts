...
/**
 * Surface material applied to either side of the sheet:
 * - "color": the tiled paper pattern tinted by that side's color picker.
 ...
 * - "custom": a user-picked local png/jpg, UV-mapped the same way.
 *
 * "paper" and "banner" are built-in textures; "custom" is a picked image.
 */
export type PaperMaterial = "color" | "paper" | "banner" | "custom";
...
export interface Paper {
  id: number;
  style: PaperStyle;
  materials: Record<PaperSide, PaperMaterial>;
  pos: Vec2;
  rot: number;
  scale: number;
  faces: Face[];
...
export function makePaper(
  ...
  return {
    ...
    style,
    materials: { front: "color", back: "color" },
    pos: { x, y },
    rot: 0,
    scale: 1,
